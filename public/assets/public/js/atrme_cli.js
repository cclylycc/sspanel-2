...
TerminalShell.commands['login']=function(terminal){
	window.location = "http://ss.domeyi.com/auth/login";
}
...
Filesystem = {
	'welcome.txt': {type:'file', read:function(terminal) {
		terminal.print();
		terminal.print($('<h4>').text('~Welcome to the Test Center line entrance~'));
	...
	'about.txt': {type:'file', read:function(terminal) {
		...
		terminal.print();
		$.each([
			'This is the homepage, and also the command line entrance of domeyi.com.',
			'domeyi.com is the personal website of mine, here you can find the way',
			'to my blogs. For more about me,',
...
Filesystem['blog'] = linkFile('https://blog.domeyi.com/');
Filesystem['AboutMe'] = linkFile('https://blog.domeyi.com/about');
...
TerminalShell.commands['cat'] = function(terminal, path) {
	if (path in this.pwd) {
		...
		terminal.print('Welcome to my blog!');
	}
};
...
TerminalShell.commands['wget'] = TerminalShell.commands['curl'] = function(terminal, dest) {
	if (dest == "blog") {
		...
		var browser = $('<div>')
			.addClass('browser')
			.append($('<iframe>')
					.prop('src', "https://blog.domeyi.com").width("100%").height(600)
					.one('load', function() {
						terminal.setWorking(false);
					}));
		...
		var browser = $('<div>')
			.addClass('browser')
			.append($('<iframe>')
					.prop('src', "https://blog.domeyi.com/about").width("100%").height(600)
					.one('load', function() {
						terminal.setWorking(false);
					}));
		terminal.print(browser);
